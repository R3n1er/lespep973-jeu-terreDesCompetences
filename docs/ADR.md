...
- Helper `resolveThemeForChallenge` (`src/lib/theme.ts`) calcule le thème d’un défi à partir de son ID ou du métier cible (mapping issu de `METIERS_ADPEP`), assurant un fond/halo cohérent même si les données sont incomplètes.
